...
// O teste de hardware anterior funcionou com limiar 300.
...
void mostrarOperacao() {
  ...
  } else display.println("Aguardando usuario");
  display.setCursor(0, 31); display.print("Passagens: "); display.println(passagens);
  display.setCursor(0, 44); display.print("Peso: "); display.print(pesoGramas, 0); display.println(" g");
  display.setCursor(0, 57); display.print("Pontos: "); display.println(pontos);
  display.display();
}
...
bool testarBanco() {
  ...
  http.end();
  if (code == 200) { Serial.println("Supabase conectado."); return true; }
  Serial.print("ERRO Supabase HTTP: "); Serial.println(code);
...
void consultarSessao() {
  ...
  } else if (doc.is<JsonObject>()) item = doc.as<JsonObject>();

  if (item.isNull() || item["session_id"].isNull()) {
    if (sessaoAtiva) {
      sessaoAtiva = false; sessionId = ""; userId = ""; userName = ""; eventId = "";
      passagens = 0; pontos = 0;
...
bool registrarTampa(float peso) {
  ...
  if (code >= 200 && code < 300) {
    ...
    Serial.print("Peso enviado: "); Serial.print(peso, 2); Serial.println(" g");
    mostrarOperacao();
    return true;
  }
  Serial.print("ERRO ao registrar coleta. HTTP: "); Serial.println(code);
  Serial.println(response);
  return false;
}

void conectarWiFi() {
  Serial.println(); Serial.print("Conectando ao Wi-Fi: "); Serial.println(WIFI_SSID);
  WiFi.mode(WIFI_STA); WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
  unsigned long inicio = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - inicio < 20000) { delay(500); Serial.print("."); }
  Serial.println();
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("Wi-Fi conectado.");
    Serial.print("IP: "); Serial.println(WiFi.localIP());
    Serial.print("MAC: "); Serial.println(WiFi.macAddress());
  } else Serial.println("ERRO: Wi-Fi nao conectado.");
}

void setup() {
  Serial.begin(115200); delay(500); Wire.begin(SDA_PIN, SCL_PIN);
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDRESS)) {
    Serial.println("ERRO: OLED nao encontrado."); while (true) delay(1000);
  }
  analogReadResolution(12);
  analogSetPinAttenuation(PINO_LDR, ADC_11db);
  analogSetPinAttenuation(PINO_POT, ADC_11db);

  // Importante: inicia o estado do LDR com a leitura real para nao contar falsa passagem.
  valorLDR = analogRead(PINO_LDR);
  estadoAnteriorLDR = (valorLDR < LIMIAR_LDR);

  mostrarTela("TAMPAE", "Iniciando...");
  Serial.println("========================================");
  Serial.println("TAMPAE - INTEGRACAO + LDR + BALANCA");
  Serial.println("========================================");
  Serial.print("LDR inicial: "); Serial.println(valorLDR);
  Serial.print("Limiar LDR: "); Serial.println(LIMIAR_LDR);

  conectarWiFi();
  if (WiFi.status() == WL_CONNECTED) {
    mostrarTela("Wi-Fi conectado", "Testando banco...");
    if (testarBanco()) mostrarTela("Banco conectado", "Aguardando app...");
    else mostrarTela("Banco: ERRO", "Verifique conexao");
  } else mostrarTela("Wi-Fi: ERRO", "Verifique rede");
  delay(1000); mostrarOperacao();
}

void loop() {
  if (WiFi.status() != WL_CONNECTED) {
    static unsigned long ultimaTentativaWiFi = 0;
    if (millis() - ultimaTentativaWiFi >= 10000) { ultimaTentativaWiFi = millis(); conectarWiFi(); }
    delay(100); return;
  }

  // Sensores sao lidos continuamente, mesmo sem sessao.
  ...
  // OLED atualizado continuamente. Assim o peso muda ao girar o potenciometro.
  ...
  // Detecta apenas a transicao livre -> bloqueado.
  if (sessaoAtiva && objetoDetectado && !estadoAnteriorLDR) {
    unsigned long agora = millis();
    if (agora - ultimaPassagem >= DEBOUNCE_MS) {
      Serial.print("PASSAGEM LDR | valor: "); Serial.print(valorLDR);
      Serial.print(" | peso: "); Serial.print(pesoGramas, 2); Serial.println(" g");
      if (registrarTampa(pesoGramas)) ultimaPassagem = agora;
    }
  }
