...
bool sessaoEstavaAtivaNaUltimaConsulta = false;
...
String chamarRPC(const char* nome, const String& body, int* codigoOut = nullptr) {
  ...
  Serial.print("RPC "); Serial.print(nome); Serial.print(" HTTP: "); Serial.println(codigo);
  if (codigo < 200 || codigo >= 300) {
    Serial.print("RPC resposta: "); Serial.println(resposta);
  }
  return resposta;
}
...
String makeQrSvg() {
  ...
  if (esp_qrcode_generate(&config, qrPayload.c_str()) != ESP_OK || capture.size <= 0) return "<p>Erro ao gerar QR.</p>";
  const int margem = 4;
  ...
  for (int y = 0; y < capture.size; y++) for (int x = 0; x < capture.size; x++) if (capture.modules[y][x]) {
...
void handleRoot() {
  ...
  page += "<meta name='viewport' content='width=device-width,initial-scale=1'><title>TampAê - Teste</title><style>";
  ...
  page += svg;
  page += "<div class='title'>TAMPAÊ - TESTE</div><div class='hint'>Escaneie para iniciar a sessão</div><div class='id'>" + machineId + "</div></main></body></html>";
  server.send(200, "text/html; charset=utf-8", page);
}

void handleInfo() {
  String json = String("{\"machine_id\":\"") + machineId + "\",\"event_id\":\"" + EVENT_ID + "\",\"ip\":\"" + WiFi.localIP().toString() + "\"}";
...
bool lerSessaoAtiva(String& novaId, String& novoUser, String& novoNome, String& novoEvento) {
  ...
  JsonDocument doc;
  if (deserializeJson(doc, resposta) || !doc.is<JsonArray>()) return false;
  JsonArray a = doc.as<JsonArray>();
  if (!a.size()) return false;
...
void iniciarNovaSessao(const String& id, const String& uid, const String& nome, const String& evento) {
  ...
  sessaoAtiva = true; envioFinalEmAndamento = false; sessaoEstavaAtivaNaUltimaConsulta = true;
  zerarAcumulador();
  Serial.println();
  Serial.println("========================================");
  Serial.println("USUARIO CONECTADO");
  Serial.print("NOME: "); Serial.println(nomeUsuario.length() ? nomeUsuario : userId);
  Serial.print("EVENTO_ID: "); Serial.println(eventoSessao.length() ? eventoSessao : "NULL");
...
void detectarBalanca() {
  float peso = lerPeso();
  if (ldrBloqueado) { inicioPesoEstavel = 0; return; }
  if (peso <= BALANCA_RESET_GRAMAS) { balancaCargaRegistrada = false; inicioPesoEstavel = 0; return; }
  if (balancaCargaRegistrada) return;
  if (peso < BALANCA_MIN_GRAMAS) { inicioPesoEstavel = 0; return; }
  if (inicioPesoEstavel == 0) { inicioPesoEstavel = millis(); return; }
  ...
  Serial.print("Peso desta carga: "); Serial.print(peso, 1); Serial.println(" g");
  Serial.print("Regra: "); Serial.print(quantidade); Serial.println(" tampinha(s) = ceil(peso / 13)");
  Serial.print("TOTAL TAMPINHAS: "); Serial.println(totalTampinhas);
...
bool encerrarSessaoNaMaquina() {
  ...
  for (int tentativa = 1; tentativa <= 3; tentativa++) {
    ...
    Serial.print("ENCERRAMENTO DA SESSAO - tentativa "); Serial.print(tentativa); Serial.print(" HTTP: "); Serial.println(codigo);
    if (codigo >= 200 && codigo < 300) {
      Serial.print("Resposta encerrar_sessao_maquina: "); Serial.println(resposta);
      return true;
    }
    delay(300);
  }
...
bool enviarResultadoEFecharSessao() {
  ...
  Serial.print("PASSAGENS/CARGAS: "); Serial.println(totalPassagens);
  ...
  // IMPORTANTE: 0.00 nao significa "sem pesagem". A constraint do banco
  // diferencia unidade de pesagem e rejeita a combinacao de unidade com peso 0.
  // Se nenhuma carga foi medida pela balanca, enviamos NULL.
  if (totalPeso > 0) req["p_peso_real_gramas"] = (double)totalPeso;
  else req["p_peso_real_gramas"] = nullptr;

  req["p_peso_estimado_gramas"] = nullptr;

  String body; serializeJson(req, body);
  Serial.print("BODY FINAL: "); Serial.println(body);
  int codigo = 0;
  String resposta = chamarRPC("registrar_coleta", body, &codigo);
  if (codigo < 200 || codigo >= 300) {
    Serial.println("ERRO: coleta final nao gravada. Sessao continua ativa.");
    envioFinalEmAndamento = false;
    return false;
  }
  Serial.println("COLETA FINAL ENVIADA COM SUCESSO");
  Serial.print("COLLECTION_ID/RESPOSTA: "); Serial.println(resposta);
  bool sessaoFechada = encerrarSessaoNaMaquina();
  if (!sessaoFechada) {
    Serial.println("ERRO: coleta gravada, mas nao foi possivel confirmar o encerramento.");
    envioFinalEmAndamento = false;
    return false;
  }
  Serial.println("SESSAO ENCERRADA COM SUCESSO");
  sessaoAtiva = false;
  sessaoEstavaAtivaNaUltimaConsulta = false;
  envioFinalEmAndamento = false;
  sessionId = ""; userId = ""; nomeUsuario = ""; eventoSessao = "";
  zerarAcumulador();
  mostrarAguardando();
  return true;
}

void finalizarSessaoCanceladaPeloCelular() {
  Serial.println();
  Serial.println("========================================");
  Serial.println("SESSAO ENCERRADA PELO CELULAR");
  Serial.println("Nenhuma registrar_coleta adicional sera criada.");
  Serial.println("Limpando sessao local e voltando para a tela inicial.");
  Serial.println("========================================");
  sessaoAtiva = false;
  sessaoEstavaAtivaNaUltimaConsulta = false;
  envioFinalEmAndamento = false;
...
void verificarBotao() {
  ...
  if (botaoAnterior == HIGH && atual == LOW && millis() - ultimoBotao >= DEBOUNCE_BOTAO) {
    ...
    } else Serial.println("BOTAO pressionado, mas nao existe sessao ativa.");
  }
  botaoAnterior = atual;
}
...
void verificarSessao() {
  ...
  if (encontrou) {
    if (!sessaoAtiva) {
      ...
      Serial.println("A sessao atual continua sendo usada ate finalizar.");
    }
    sessaoEstavaAtivaNaUltimaConsulta = true;
  } else {
    // So consideramos cancelamento remoto quando antes havia uma sessao ativa.
    // Uma falha de HTTP/Supabase nao chega aqui: lerSessaoAtiva retorna false
    // tanto para erro quanto para array vazio, por isso fazemos uma segunda
    // consulta curta antes de limpar a sessao.
    if (sessaoAtiva && sessaoEstavaAtivaNaUltimaConsulta) {
      delay(80);
      String confirmaId, confirmaUser, confirmaNome, confirmaEvento;
      bool confirmou = lerSessaoAtiva(confirmaId, confirmaUser, confirmaNome, confirmaEvento);
      if (!confirmou) finalizarSessaoCanceladaPeloCelular();
      else sessaoEstavaAtivaNaUltimaConsulta = true;
    } else if (!sessaoAtiva) {
      mostrarAguardando();
    }
  }
...
void setup() {
  ...
  if (!conectarWiFi()) { oledMessage("TAMPAE", "Falha Wi-Fi"); return; }
  if (!criarMaquina()) { oledMessage("TAMPAE", "Erro maquina"); return; }
  iniciarServidor();
  mostrarAguardando();
  Serial.println("========================================");
  Serial.println("PRONTO PARA TESTE");
  Serial.print("Abra no celular: http://"); Serial.println(WiFi.localIP());
